...
# Vercel's environment may reuse the same Python process for multiple requests.
# We create a single, long-running event loop to avoid `Event loop is closed` errors.
try:
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            ...
            if vercel_url:
                try:
                    message = loop.run_until_complete(set_telegram_webhook(vercel_url))
                    self.send_response(200)
                    self.send_header("Content-type", "application/json")
                    self.end_headers()
