...
@router.callback_query(F.data.startswith("admin:dm_prompt"))
...
@router.callback_query(F.data.startswith("admin:quota_prompt"))
...
async def on_simple(cb: CallbackQuery, bot: Bot):
    ...
    if user_packs:
        ...
        await cb.message.answer("می‌خواهید استیکر جدید را به کدام پک اضافه کنید؟", reply_markup=pack_selection_kb(uid, "simple"))
    else:
...
async def on_ai(cb: CallbackQuery, bot: Bot):
    ...
    if user_packs:
        ...
        await cb.message.answer("می‌خواهید استیکر جدید را به کدام پک اضافه کنید؟", reply_markup=pack_selection_kb(uid, "ai"))
    else:
...
async def on_simple_confirm(cb: CallbackQuery, bot: Bot):
    ...
    img = render_image(text=simple_data["text"] or "سلام", v_pos="center", h_pos="center", font_key="Default", color_hex="#FFFFFF", size_key="medium", bg_mode=simple_data.get("bg_mode") or "transparent", bg_photo=simple_data.get("bg_photo_bytes"), as_webp=True)
    s["last_sticker"] = img
...
async def on_ai_type(cb: CallbackQuery, bot: Bot):
    ...
    if sticker_type == "image":
        ...
        await cb.message.answer("قابلیت ویدیو در این نسخه فعال نیست.", reply_markup=back_to_menu_kb(cb.from_user.id == ADMIN_ID))
    await cb.answer()
...
async def on_ai_size(cb: CallbackQuery, bot: Bot):
    ...
    img = render_image(text=ai_data.get("text") or "متن ساده", v_pos=ai_data["v_pos"], h_pos=ai_data["h_pos"], font_key="Default", color_hex=ai_data["color"], size_key=size, bg_mode="transparent", bg_photo=ai_data.get("bg_photo_bytes"), as_webp=False)
    file_obj = BufferedInputFile(img, filename="preview.png")
    await cb.message.answer_photo(file_obj, caption="پیش‌نیمایش آماده است", reply_markup=after_preview_kb("ai"))
    await cb.answer()
...
async def on_ai_confirm(cb: CallbackQuery, bot: Bot):
    ...
    if left <= 0 and not is_admin:
        ...
        return
    ai_data = sess(cb.from_user.id)["ai"]
    img = render_image(text=ai_data.get("text") or "سلام", v_pos=ai_data["v_pos"], h_pos=ai_data["h_pos"], font_key="Default", color_hex=ai_data["color"], size_key=ai_data["size"], bg_mode="transparent", bg_photo=ai_data.get("bg_photo_bytes"), as_webp=True)
...
async def on_rate_yes(cb: CallbackQuery, bot: Bot):
    ...
    if not sticker_bytes or not pack_short_name:
        ...
        return
    try:
        sticker_to_add = InputSticker(sticker=BufferedInputFile(sticker_bytes, filename="sticker.webp"), emoji_list=["😂"])
        await cb.bot.add_sticker_to_set(user_id=cb.from_user.id, name=pack_short_name, sticker=sticker_to_add)
...
async def on_pack_start_creation(cb: CallbackQuery, bot: Bot):
    ...
    mode = s.get("pack_wizard", {}).get("mode", "simple")
    s["pack_wizard"] = {"step": "awaiting_name", "mode": mode}
    rules_text = ("برای ایجاد پک جدید، یک نام انگلیسی ارسال کنید.\n\n• فقط حروف انپلیسی کوچک، عدد و زیرخط\n• حداکثر ۵۰ کاراکتر")
    await cb.message.answer(rules_text, reply_markup=back_to_menu_kb(cb.from_user.id == ADMIN_ID))
    await cb.answer()
...
async def on_message(message: Message, bot: Bot):
    ...
    if pack_wizard.get("step") == "awaiting_name" and message.text:
        ...
        if any(word in pack_name_lower for word in FORBIDDEN_WORDS):
            await message.answer("نام پک انتخاب شده نامناسب است. لطفاً از کلمات مناسب استفاده کنید.", reply_markup=back_to_menu_kb(is_admin))
            return
        if not is_valid_pack_name(pack_name):
            await message.answer("نام پک نامعتبر است. لطفا طبق قوانین یک نام جدید انتخاب کنید.", reply_markup=back_to_menu_kb(is_admin))
            return
        short_name = f"{pack_name}_by_{BOT_USERNAME}"
        mode = pack_wizard.get("mode")
        if len(short_name) > 64:
            await message.answer(f"نام پک خیلی طولانی است. لطفا یک نام کوتاه‌تر انتخاب کنید.", reply_markup=back_to_menu_kb(is_admin))
            return
        try:
            pack_exists = await check_pack_exists(bot, short_name)
            if pack_exists:
                ...
                sticker_to_add = InputSticker(sticker=BufferedInputFile(dummy_img, filename="sticker.webp"), emoji_list=["🎉"])
                try:
                    await bot.create_new_sticker_set(user_id=uid, name=short_name, title=pack_name, stickers=[sticker_to_add], sticker_type='regular', sticker_format='static')
                ...
                s["current_pack_short_name"] = short_name
                s["current_pack_title"] = s.get("current_pack_title", pack_name)
                s["pack_wizard"] = {}
                add_user_pack(uid, pack_name, short_name)
            ...
            if mode == "simple":
                s["mode"] = "simple"
                s["simple"] = {"text": None, "bg_mode": "transparent", "save_sticker": None}
