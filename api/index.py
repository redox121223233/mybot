...
# This is the key to solving the conflict between Vercel's sync handler and aiogram's async code.
nest_asyncio.apply()
...
class handler(BaseHTTPRequestHandler):
    """
    Vercel serverless function handler. This class structure is required by Vercel.
    """
    def do_GET(self):
        # We run the async logic inside the sync handler method using asyncio.run().
        # nest_asyncio allows this to work without event loop conflicts.
        asyncio.run(self.handle_get_async())
    ...
    def do_POST(self):
        # We run the async logic inside the sync handler method using asyncio.run().
        asyncio.run(self.handle_post_async())
