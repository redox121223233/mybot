...
def pack_selection_kb(uid: int, mode: str):
    ...
    # اگر پک قبلی وجود داشته باشد، آن را به عنوان گزینه اول نمایش می‌دهیم
...
async def on_simple(cb: CallbackQuery, bot: Bot):
    ...
    if user_packs:
        ...
        rules_text = (
            ...
            "• حداکثر ۵۰ کاراکتر (به دلیل اضافه شدن نام ربات)"
        )
...
# --- تابع اصلاح شده با راه حل نهائی ---
...
async def on_rate_yes(cb: CallbackQuery, bot: Bot):
    ...
    try:
        sticker_to_add = InputSticker(
            ...
            emoji_list=["😂"]  # تغییر ایموجی به یک گزینه راج‌تر
        )
        ...
        await cb.message.answer(f"خط در افزودن استیکر به پک: {e.message}", reply_markup=back_to_menu_kb(cb.from_user.id == ADMIN_ID))
    except Exception as e:
        print(f"DEBUG: Unexpected error on add_sticker_to_set: {e}")
        traceback.print_exc()
        await cb.message.answer(f"خطای غیرمنتظره رخ داد. لطفا به ادمین اطلاع دهید.\nخطا: {str(e)}", reply_markup=back_to_menu_kb(cb.from_user.id == ADMIN_ID))
        ...
        try:
            ...
            if pack_exists:
                ...
                if mode == "simple":
                    s["mode"] = "simple"
                    s["simple"] = {"text": None, "bg_mode": "transparent", "bg_photo_bytes": None}
                    await message.answer("متن استیکر ساده را بفرست:", reply_markup=back_to_menu_kb(is_admin))
                elif mode == "ai":
                    s["mode"] = "ai"
                    s["ai"] = {
                        "text": None, "v_pos": "center", "h_pos": "center", "font": "Default",
