...
class StickerManager:
    ...
    def process_sticker_photo(self, user_id, file_id):
        ...
        try:
            ...
            self.api.send_message(
                user_id,
                "✍️ متن مورد نظر برای استیکر را وارد کنید:",
                reply_markup={"keyboard": [["⬅️ بازگشت"]], "resize_keyboard": True}
            )
        except Exception as e:
            logger.error(f"خطا در دانلود عکس: {e}")
            self.api.send_message(user_id, "❌ مشکلی در دریافت عکس پیش آمد.")
    ...
    def add_text_to_sticker(self, user_id, text):
        ...
        if not photo_path or not pack_name:
            self.api.send_message(user_id, "❌ مشکلی در جریان استیکرسازی پیش آمد.")
            return
        ...
        try:
            ...
            image.save(final_path, "WEBP")

            # ساخت پک یا اضافه کردن استیکر
            ...
            # ارسال استیکر ساخته شده به کاربر
