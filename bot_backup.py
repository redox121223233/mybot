...
def wrap_text_to_width(draw: ImageDraw.ImageDraw, text: str, font: ImageFont.FreeTypeFont, max_width: int) -> List[str]:
    ...
    # اگر متن چند خطی است، هر خط را جداگانه پردازش می‌کنیم
    if '\n' in text:
        all_lines = []
        for line in text.split('\n'):
            line = line.strip()
            if line:
                wrapped_lines = wrap_text_to_width(draw, line, font, max_width)
                all_lines.extend(wrapped_lines)
        return all_lines if all_lines else [text]
    
    # اگر متن خیلی کوتاه است، برگردان به صورت تک خطی
    try:
    ...
    for word in words:
        # تست اضافه کردن کلمه بعدی به خط فعلی
        test_line = (current_line + " " + word).strip() if current_line else word
        
        # اگر خط جدید در محدوده عرض است، اضافه می‌کنیم
        try:
            if draw.textlength(test_line, font=font) <= max_width:
                current_line = test_line
            else:
                # اگر خط فعلی خالی نیست، ذخیره می‌کنیم
                if current_line:
                    lines.append(current_line)
                    current_line = word
                else:
                    # اگر کلمه خیلی طولانی است، مجبوریم بشکنیم
                    lines.append(word)
                    current_line = ""
        except ValueError:
            # در صورت خطا، کلمه را به عنوان خط جداگانه اضافه می‌کنیم
            if current_line:
                lines.append(current_line)
            lines.append(word)
            current_line = ""
