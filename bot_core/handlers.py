...
async def on_simple(cb: CallbackQuery, bot: Bot):
    ...
    s["mode"] = "simple"
    s["simple"] = {"awaiting_text": True, "text": None, "bg_mode": "transparent", "bg_photo_bytes": None}
    await cb.message.edit_text("متن استیکر ساده رو بفرست:", reply_markup=back_to_menu_kb(cb.from_user.id == ADMIN_ID))
    await cb.answer()
...
async def on_ai(cb: CallbackQuery, bot: Bot):
    ...
    if left <= 0 and not is_admin:
        await cb.message.edit_text(
            "سهمیه استیکرساز پیشرفته شما تمام شده است. می‌توانید از استیکرساز ساده استفاده کنید.",
            reply_markup=back_to_menu_kb(is_admin)
        )
        await cb.answer()
        return
    ...
    s["mode"] = "ai"
    s["ai"] = {
        "awaiting_text": True, "text": None, "v_pos": "center", "h_pos": "center", "font": "Default",
        "color": "#FFFFFF", "size": "large", "bg_photo_bytes": None, "bold": False
    }
    await cb.message.edit_text("متن استیکر پیشرفته رو بفرست:", reply_markup=back_to_menu_kb(cb.from_user.id == ADMIN_ID))
    await cb.answer()

# Handlers for advanced sticker creation flow
...
async def on_ai_confirm(cb: CallbackQuery, bot: Bot):
    ...
    ai_data = sess(cb.from_user.id)["ai"]
    img = render_image(
        text=ai_data.get("text"), v_pos=ai_data["v_pos"], h_pos=ai_data["h_pos"],
        font_key=ai_data["font"], color_hex=ai_data["color"], size_key=ai_data["size"],
        bg_photo=ai_data.get("bg_photo_bytes"), as_webp=True, bold=ai_data["bold"]
    )
...
async def on_message(message: Message, bot: Bot):
    ...
    if mode == "simple" and s["simple"].get("awaiting_text"):
    ...
    if mode == "ai" and s["ai"].get("awaiting_text"):
