...
def ask_text_kb(prefix: str):
    kb = InlineKeyboardBuilder()
    kb.button(text="بله، متن اضافه کن", callback_data=f"{prefix}:text:yes")
    kb.button(text="خیر، بدون متن", callback_data=f"{prefix}:text:no")
    kb.adjust(1)
    return kb.as_markup()

def ai_font_kb():
    from .utils.image_processing import LOCAL_FONT_FILES
    kb = InlineKeyboardBuilder()
    for font_name in LOCAL_FONT_FILES.keys():
        if font_name != "Default":
            kb.button(text=font_name, callback_data=f"ai:font:{font_name}")
    kb.adjust(2)
    return kb.as_markup()

def pack_type_selection_kb(mode: str):
    kb = InlineKeyboardBuilder()
    kb.button(text="پک استیکر معمولی (عکس)", callback_data=f"pack:type:static:{mode}")
    kb.button(text="پک استیکر ویدیویی", callback_data=f"pack:type:video:{mode}")
    kb.adjust(1)
    return kb.as_markup()
