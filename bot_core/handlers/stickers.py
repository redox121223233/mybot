...
from ..keyboards import (
    ...
    back_to_menu_kb, main_menu_kb, ask_text_kb, ai_font_kb, pack_type_selection_kb
)
    ...
    if action == "select":
        ...
        if pack:
            ...
            if mode == "simple":
                ...
                await safe_edit_text(cb, f"پک «{pack['name']}» انتخاب شد. آیا می‌خواهید متنی روی استیکر باشد؟", reply_markup=ask_text_kb("simple"))
            else:
        ...
        await safe_edit_text(cb, "نوع پکی که می‌خواهید بسازید را انتخاب کنید:", reply_markup=pack_type_selection_kb(mode))
...
async def on_simple_actions(cb: CallbackQuery, bot: Bot):
    ...
    if action == "text":
        choice = parts[2]
        if choice == "yes":
            storage.update_session(uid, {"mode": "simple_awaiting_text"})
            await safe_edit_text(cb, "متن استیکر را بفرستید:")
        else:
            simple_data["text"] = None
        ...
        if bg_mode == "photo_prompt":
            ...
            storage.update_session(uid, {"simple": simple_data})
            text = simple_data.get("text") or ""
            img = render_image(text, "center", "center", "Default", "#FFFFFF", "medium", bg_mode=bg_mode)
            await cb.message.answer_photo(BufferedInputFile(img, "p.png"), caption="پیش‌نمایش:", reply_markup=after_preview_kb("simple"))

    elif action == "confirm":
        text = simple_data.get("text") or ""
        img = render_image(text, "center", "center", "Default", "#FFFFFF", "medium",
        ...
        await safe_edit_text(cb, "آیا می‌خواهید متنی روی استیکر باشد؟", reply_markup=ask_text_kb("simple"))
...
async def on_ai_actions(cb: CallbackQuery, bot: Bot):
    ...
    if action == "type":
        ...
        storage.update_session(uid, {"ai": ai_data})
        await safe_edit_text(cb, "موقعیت عمودی متن:", reply_markup=ai_vpos_kb())
        ...
        img = render_image(text, ai_data.get("v_pos", "center"), ai_data.get("h_pos", "center"),
        ...
        if sticker_type == "video":
            ...
            if video_bytes:
                text_overlay = None
                if ai_data.get("text"):
                    text_overlay = {k: ai_data.get(k) for k in ["text", "v_pos", "h_pos", "color", "size", "font"]}
                    text_overlay["font_key"] = text_overlay.pop("font", "Default")
        ...
        try:
            ...
            if format == "video":
                ...
                if s.get("mode") == "simple":
                    d = s.get("simple", {})
                    png = render_image(d.get("text") or "", "center", "center", "Default", "#FFFFFF", "medium", bg_mode=d.get("bg_mode"), bg_photo=d.get("bg_photo_bytes"), as_webp=False)
                else:
                    d = s.get("ai", {})
                    png = render_image(d.get("text") or "", d.get("v_pos", "center"), d.get("h_pos", "center"),
                                      d.get("font", "Default"), d.get("color", "#FFFFFF"), d.get("size", "medium"),
                                      bg_photo=d.get("bg_photo_bytes"), as_webp=False)
...
async def on_message(message: Message, bot: Bot):
    ...
    if s.get("pack_wizard", {}).get("step") == "awaiting_name" and message.text:
        ...
        try:
            ...
            pack_type = s["pack_wizard"].get("type", "static")
            await bot.create_new_sticker_set(uid, short_name, pack_name, stickers=[sticker], sticker_format=pack_type)
            ...
            if mode == "simple":
                await message.answer(f"پک «{pack_name}» ساخته شد! آیا می‌خواهید متنی روی استیکر باشد؟", reply_markup=ask_text_kb("simple"))
            else:
    ...
    if message.photo:
        ...
        if s.get("mode") == "simple" and s_simple.get("awaiting_bg_photo"):
            ...
            try:
                ...
                text = s_simple.get("text") or ""
                img = render_image(text, "center", "center", "Default", "#FFFFFF", "medium", bg_photo=s_simple["bg_photo_bytes"])
            ...
            try:
                ...
                storage.update_session(uid, {"ai": s_ai})
                await message.answer("عکس دریافت شد. آیا می‌خواهید متنی روی آن اضافه کنید؟", reply_markup=ask_text_kb("ai"))
            finally:
                storage.update_session(uid, {"is_processing": False})
            return
    ...
    if message.video or message.animation:
        ...
        try:
            ...
            storage.update_session(uid, {"ai": ai_data})
            await message.answer("فایل دریافت شد. آیا می‌خواهید متنی روی آن اضافه کنید؟", reply_markup=ask_text_kb("ai"))
        finally:
            storage.update_session(uid, {"is_processing": False})
        return
    ...
    if message.text:
        ...
        mode = s.get("mode")
        if mode == "simple_awaiting_text":
            storage.update_session(uid, {"simple": {"text": message.text.strip()}, "mode": "simple"})
            await message.answer("پس‌زمینه را انتخاب کنید:", reply_markup=simple_bg_kb())
            return

        if mode in ["ai_awaiting_text_for_image", "ai_awaiting_text_for_overlay"]:
            ai_data = s.get("ai", {})
            ai_data["text"] = message.text.strip()
            storage.update_session(uid, {"ai": ai_data, "mode": "ai"})
            await message.answer("انتخاب فونت:", reply_markup=ai_font_kb())
            return
